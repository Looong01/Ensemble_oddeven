...
noise=0
for i in range(9000):
    final_img, final_label, even_num = generate_imgs(data_train, labels_train, noise, img_shape=(28, 28))
...
noise_val=0
for i in range(3000):
    final_img, final_label, even_num = generate_imgs(data_val, labels_val, noise_val, img_shape=(28, 28))
    im = Image.fromarray(final_img.astype(np.uint8))
    if im.mode == "F":
        im = im.convert("RGB")
    if final_label == "even":
...
noise_test=0
for i in range(1500):
    final_img, final_label, even_num = generate_imgs(data_test, labels_test, noise_test, img_shape=(28, 28))
    im = Image.fromarray(final_img.astype(np.uint8))
    if im.mode == "F":
        im = im.convert("RGB")
    if final_label == "even":
